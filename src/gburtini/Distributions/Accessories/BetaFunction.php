<?php
	...
	class BetaFunction {
		...
		public static function inverseIncompleteBetaFunction($p, $a, $b) {
			...
			$afac = -GammaFunction::logGammaFunction($a) - GammaFunction::logGammaFunction($b) + GammaFunction::logGammaFunction($a + $b);	// TODO: check that this isn't something else in the class
			for(; $j < 10; $j++) {
				if ($x === 0 || $x === 1)
					return $x;
				$err = static::incompleteBetaFunction($x, $a, $b) - $p;
			...
			if ($x < (($a + 1.0)/($a + $b + 2.0)))
				return $bt* static::continuedFraction($a,$b,$x)/$a;
			else
				return 1 - ($bt*static::continuedFraction($b,$a,1.0-$x)/$b);
		}
